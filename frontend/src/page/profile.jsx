...
const UserInfo = (props) => {
  return (
    <div style={{ display: 'flex', width: '80vw', justifyContent: 'space-between', marginBottom: 20, marginTop: 20 }}>
      ...
      <div className={"blackFixedButton"} style={{ height: 40, fontSize: 24, width: 200 }} onClick={() => { props.setView('editProfile') }}>EDIT PROFILE</div>
    </div>
  )
}
...
export default function Profile(prop) {
            ...
            <div key={index} style={{ display: 'flex', width: '80vw', justifyContent: 'space-between', cursor: 'pointer' }}>
              ...
              <div className="rowCenter" style={{ columnGap: 10 }}>
                <Edit onClick={() => handleEditClick(index)} />
                {
                  list.status === 0
                    ?
                    <Lock onClick={() => handleLockClick(index)} />
                    :
                    <Unlock onClick={() => handleLockClick(index)} />
                }
                <Trash onClick={() => { handleDeleteWishList(index, list.id) }} />
              </div>
            </div >
          }
        </div>
    ...
    const parts = followings.map((following, index) => {
      return (
        <div key={index} style={{ display: 'flex', width: '80vw', justifyContent: 'space-between', marginBottom: 20, marginTop: 20 }}>
          <div className="rowCenter" onClick={() => { visitOtherUserProfile(following.id) }}>
            <User2 />
  ...
  const Followers = () => {
    ...
    const parts = followers.map((follower, index) => {
      return (
        <div key={index} style={{ display: 'flex', width: '80vw', justifyContent: 'space-between', marginBottom: 20, marginTop: 20 }}>
          <div className="rowCenter" onClick={() => { visitOtherUserProfile(follower.id) }}>
            <User2 />
